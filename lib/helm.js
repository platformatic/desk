 ...
 */
export async function apply (releaseName, chart, valueFilePaths = [], version = null, { k8s, context }) {
  const args = [
    'upgrade',
    '--install',
    releaseName,
  ...
  try {
    ...
    if (!errorDetail.apiVersion || !errorDetail.kind) {
      throw new Error(`Helm install failed for '${releaseName}': ${err.output}`)
    }
    ...
    await kubectl.waitByName('crd', crdName, 'established', { context })
    return apply(releaseName, chart, valueFilePaths, version, { k8s, context })
  }
}
...
function parseNotes (output) {
  ...
  try {
    ...
    debug(`Failed to parse NOTES JSON: ${err.message}`)
    return null
  }
}

async function getCrdName (kind, apiVersion, retries = 30) {
  debug({ kind, apiVersion })
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    ...
    if (crdName) return crdName

    debug(`CRD for ${kind} in ${group} not found yet, retrying (${attempt + 1}/${retries})`)
    await setTimeout(5000)
  }

  throw new Error(`CRD for kind "${kind}" in group "${group}" not found after ${retries} retries`)
}
