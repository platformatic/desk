...
export async function installInfra (charts, { context }) {
  ...
  // Split charts into those with dependencies and those without
  const independent = chartList.filter(([, config]) => !config.dependsOn?.length)
  const dependent = chartList.filter(([, config]) => config.dependsOn?.length)

  const results = await Promise.all(independent.map(entry => installChart(entry, { context })))

  for (const entry of dependent) {
    results.push(await installChart(entry, { context }))
  }

  return results
}

async function installChart ([chart, config], { context }) {
  ...
  return helm.apply(releaseName, chartName, valueFilePaths, config.version, { k8s, context })
}
