...
export async function createDeployment (name, imageNameTag, namespace, envVars, dryRun, { context, version }) {
...
export async function createService (name, imageNameTag, namespace, dryRun, { context, version }) {
  const resourceName = version ? `${name}-${version}` : name
  const instanceLabel = version ? `${name}-${version}` : name
  const nameLabel = name
...
export async function createHTTPRoute (name, namespace, dryRun, { context }) {
  const httpRoute = {
