...
import { debug, clusterName } from '../utils.js'
...
export async function getStatus ({ name, context }) {
  try {
    ...
    const installCommand = formatInstallCommand({
      ...
      prometheusUrl,
      kubeContext,
      githubToken: context.secrets.PULL_SECRET_TOKEN,
      githubUser: context.secrets.PULL_SECRET_USER
    })
...
async function getPostgresConnectionString (profileName) {
  try {
    const services = await kubectl.search('svc', ['app.kubernetes.io/name=postgresql'])
    ...
    const nodePort = postgresService.spec.ports.find(port => port.name === 'tcp-postgresql')?.nodePort
...
async function getValkeyConnectionString (profileName, context) {
  try {
    ...
    const valkeyService = services.items.find(service =>
      service.metadata.name.includes('primary') ||
      service.metadata.name.includes('master') ||
      !service.metadata.name.includes('headless')
    ) || services.items[0]

    const serviceName = valkeyService.metadata.name
    const port = valkeyService.spec.ports.find(port => port.name === 'tcp-redis')?.port || 6379

    const valkeyConfig = context.dependencies['bitnamicharts/valkey']
