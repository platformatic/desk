...
export async function startCluster ({ provider, chartDir, name, platformatic }) {
  ...
  if (needsNodeImage) {
    ...
    if (platformatic.services?.workflow?.hotReload) {
      try {
        await spawn('docker', ['build', '-t', 'platformatic/workflow:dev', '-f', devDockerfile, dockerDir])
        await spawn('k3d', ['image', 'import', 'platformatic/workflow:dev', '-c', clusterName(name)])
      } catch (err) {
        // Image might already exist, continue
      }
    }

    if (platformatic.services?.ebpfSandbox?.hotReload) {
      try {
        const ebpfDockerfile = join(dockerDir, 'Dockerfile.ebpf-sandbox-dev')
        await spawn('docker', ['build', '-t', 'platformatic/ebpf-sandbox-server:dev', '-f', ebpfDockerfile, dockerDir])
        await spawn('k3d', ['image', 'import', 'platformatic/ebpf-sandbox-server:dev', '-c', clusterName(name)])
      } catch (err) {
        // Image might already exist, continue
      }
    }
  ...
  // Mount tracefs inside k3d nodes for eBPF tracepoint support.
  // k3d Docker containers don't mount tracefs by default.
  if (platformatic.services?.ebpfSandbox) {
    const nodeContainers = (await spawn('docker', [
      'ps', '--filter', `name=k3d-${clusterName(name)}-server-`, '--format', '{{.Names}}'
    ])).stdout.trim().split('\n').filter(Boolean)

    for (const container of nodeContainers) {
      await spawn('docker', ['exec', container, 'mount', '-t', 'tracefs', 'tracefs', '/sys/kernel/tracing'])
    }
  }
}
...
export async function getStatus ({ name, context }) {
  try {
    ...
    const valkeyReginaConnectionString = await getValkeyReginaConnectionString(name, context)
    ...
    if (valkeyReginaConnectionString) {
      status.valkeyRegina = {
        connectionString: valkeyReginaConnectionString
      }
    }
...
async function getValkeyReginaConnectionString (profileName, context) {
  try {
    const services = await kubectl.search('svc', ['app.kubernetes.io/instance=valkey-regina'])
    ...
    const valkeyService = services.items.find(service =>
      service.metadata.name.includes('valkey-regina') &&
      !service.metadata.name.includes('headless')
    ) || services.items[0]
    ...
    const valkeyConfig = context.dependencies['cloudpirates/valkey-regina']
    const username = valkeyConfig?.auth?.username || 'default'
    const password = valkeyConfig?.auth?.password || 'default'
