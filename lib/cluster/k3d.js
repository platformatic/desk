...
export async function startCluster ({ provider, chartDir, name, platformatic }) {
  ...
  if (needsNodeImage) {
    ...
    if (platformatic.services?.icc?.hotReload) {
      try {
        await spawn('docker', ['build', '-t', 'platformatic/icc:dev', '-f', join(dockerDir, 'icc', 'Dockerfile.dev'), dockerDir])
    ...
    if (platformatic.services?.machinist?.hotReload) {
      try {
        await spawn('docker', ['build', '-t', 'platformatic/machinist:dev', '-f', join(dockerDir, 'machinist', 'Dockerfile.dev'), dockerDir])
