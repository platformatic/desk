...
export async function assertEbpfSandboxSupport (opts = {}) {
  const platform = opts.platform || osPlatform()
  const readFileFn = opts.readFileFn || (p => readFile(p, 'utf8'))

  if (platform !== 'linux') {
    throw new Error(
      'eBPF sandbox is not supported on macOS or Windows. ' +
      'Use Lima with a BPF LSM-enabled kernel on macOS.'
    )
  }

  const lsm = (await readFileFn('/sys/kernel/security/lsm')).trim()
  if (!lsm.split(',').includes('bpf')) {
    throw new Error(
      'BPF LSM is not active on this kernel. ' +
      'Add "lsm=lockdown,capability,landlock,yama,bpf" to your kernel boot parameters.'
    )
  }
}

export async function checkEbpfSandboxSupport (opts) {
  try {
    await assertEbpfSandboxSupport(opts)
    return { name: 'ebpf-sandbox (BPF LSM)', supported: true }
  } catch (err) {
    return { name: 'ebpf-sandbox (BPF LSM)', supported: false, reason: err.message }
  }
}
