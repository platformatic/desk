...
dotenv.config({ path: join(import.meta.dirname, '..', '.env'), quiet: true, processEnv: userSecrets })
...
export async function loadContext (profileNameOrPath, options = {}) {
  ...
  if (options.command === 'up' && context.platformatic.services) {
    const requiredVars = []
    ...
    if (context.platformatic.services.workflow?.hotReload) {
      if (!userSecrets.WORKFLOW_REPO) {
        requiredVars.push('WORKFLOW_REPO')
      }
    }
    if (context.platformatic.services.ebpfSandbox?.hotReload) {
      if (!userSecrets.EBPF_SANDBOX_REPO) {
        requiredVars.push('EBPF_SANDBOX_REPO')
      }
    }

    if (requiredVars.length > 0) {
      throw new Error(
        `Profile "${profileName}" uses hotReload, but the following environment variable(s) are not set: ${requiredVars.join(', ')}\n` +
...
function getDependencyCtx (profileDependencies, configDependencies) {
  ...
  // Include config dependencies marked as default (always installed)
  for (const [chart, config] of Object.entries(configDependencies)) {
    if (chart === 'platformatic/helm') continue
    if (config.default) {
      result[chart] = { ...config }
    }
  }

  // Merge profile-requested dependencies
  for (const [chart, profileConfig] of Object.entries(profileDependencies)) {
    const defaults = configDependencies[chart]
    if (!defaults) {
      warn(`${chart} not configured in config.yaml. Skipping setup`)
      continue
    }
    result[chart] = deepmerge(defaults, profileConfig)
  }
...
function getPlatformaticCtx (profile, config) {
  ...
  // Allow overriding the helm chart location with a local path for development.
  // When set, the version is cleared so helm uses the local chart as-is.
  if (userSecrets.PLT_HELM_CHART_PATH) {
    chart.location = userSecrets.PLT_HELM_CHART_PATH
    chart.version = null
  }
